...
def heatmap_log(ax, table, cbar_label, fmt=lambda v: f"{v:,.0f}",
                ...
                cbar_labels=("1", "10", "100", "1K", "10K")):
    """Sequential-blue heatmap of a categorical x ordinal table, log colour scale.

    Every cell prints its own value, so colour is a reading aid and never the
    data itself; the ink flips to white only on the dark end of the ramp. Cells
    with no data are drawn as a dash. Returns the colorbar so callers can adjust
    it.
    """
